...
class QLState:
    """
    This class represents each grid state
    """
    def __init__(self):
        self.value = 0.0
        self.position = (0, 0)
        ...
        self.visited = 0
        self.maximum = 0.0
        self.dir_with_max = [0, 1, 2, 3]
        self.q_values = [0.0, 0.0, 0.0, 0.0]
        self.confidence = [0.0, 0.0, 0.0, 0.0]

    def get_max(self):
        """
        Returns the maximum q-value.
        """
        max_pos = int(random.choice(self.dir_with_max))
    ...
    def update(self, direction, value):
        ...
        # Loop thru all directions and update dir_with_max
        self.dir_with_max = []
        for pos in range(4):
            if self.q_values[pos] == self.maximum:
                self.dir_with_max.append(pos)
    ...
    def get_max_action(self):
        ...
        """
        return random.choice(self.dir_with_max)
...
class QLGrid:
    """
    The class representing the grid
    """
...
if len(sys.argv) >= 3:
    move_cost = float(sys.argv[2])

if len(sys.argv) >= 4:
    move_prob = float(sys.argv[3])

if len(sys.argv) >= 5:
    trials = int(sys.argv[4])
    
# Setup the grid    
mygrid = QLGrid(file_name, move_cost, move_prob)

# Execution
if trials > 0:
    mygrid.learn(trials)
else:
    mygrid.learn_20_seconds()

# Results
mygrid.print_greatest_directions()
mygrid.print_greatest_qvalues()
mygrid.print_summary()
